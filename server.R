...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$goButton, {
    ...
    for (i in 1:length(unique(case$DEFECT_NAME))) {
      ...
      V2_pop[[i]] <-
        plyr::rename(V2_pop[[i]], replace = c(POPULATION = "CASES"))
      ...
      CaseFile <- paste(paste("Defect", code, sep = "_"), ".cas", sep =
                          "")
      PopulationFile <-
        paste(paste("Defect", code, sep = "_"), ".pop", sep = "")
      
      
      ss_case[[i]] <-
        write.cas(V2_case[[i]], outputDir, paste("Defect", code, sep = "_"))
      ss_pop[[i]] <-
        write.pop(V2_pop[[i]], outputDir, paste("Defect", code, sep = "_"))
